  ...
  for(int i=0;i<n;i++)
    {
      ...
      //못만들면 큰값으로 빼서 -> 못만든다
      int money = 123456;
      while(first_money<last_money)
        {
          int mid_money = (first_money+last_money)/2;
          int spent = 123456;
          int sk_left= 0;
          ...
          for(int sk = sk_left; sk<=sk_right;sk++)
            ...
            if(mid_money - spent <0)
            {
              last_money = mid_money-1;
            }
            else
            {
              first_money = mid_money+1;
              if(money<mid_money)
              {
                money = mid_money;
              }
